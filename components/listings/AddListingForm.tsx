...
const sections = [
  ...
  },
  {
    ...
    label_ar: "التفاصيل والسعر",
    label_en: "Details & Price",
  },
  {
    id: "media",
    icon: HiOutlinePhoto,
...
const propertyTypes = [
  {
    ...
    label_ar: "شقة",
    label_en: "Apartment",
    desc_ar: "شقق سكنية بجميع المساحات",
    desc_en: "Residential apartments of all sizes",
  },
  {
    ...
    label_ar: "فيلا",
    label_en: "Villa",
    desc_ar: "فيلات مستقلة أو توين هاوس",
    desc_en: "Standalone villas or townhouses",
  },
  {
    ...
    label_ar: "تجاري",
    label_en: "Commercial",
    desc_ar: "محلات، مكاتب، ومساحات تجارية",
    desc_en: "Shops, offices, and commercial spaces",
  },
];
...
export default function AddListingForm() {
  ...
  const validateSection = (id: SectionId): string | null => {
    switch (id) {
      case "type":
        if (!form.type) return isAr ? "اختر نوع العقار" : "Select property type";
        return null;
      case "basic":
        if (!form.title_ar.trim() || !form.title_en.trim())
          return isAr ? "أدخل عنوان الإعلان بالعربية والإنجليزية" : "Enter title in Arabic and English";
        return null;
      case "location":
        if (!form.location_ar.trim() || !form.location_en.trim())
          return isAr ? "أدخل موقع العقار" : "Enter property location";
        return null;
      case "details":
        if (!form.price || Number(form.price) <= 0)
          return isAr ? "أدخل سعر صحيح" : "Enter a valid price";
        if (!form.area || Number(form.area) <= 0)
          return isAr ? "أدخل المساحة" : "Enter area";
        if (!form.baths || Number(form.baths) < 0)
          return isAr ? "أدخل عدد الحمامات" : "Enter bathrooms count";
        if (form.type !== "commercial" && (!form.beds || Number(form.beds) < 0))
          return isAr ? "أدخل عدد الغرف" : "Enter bedrooms count";
        return null;
      case "media":
        if (form.images.length === 0)
          return isAr ? "أضف صورة واحدة على الأقل" : "Add at least one image";
        return null;
      case "contact":
        if (!form.phone.trim())
          return isAr ? "أدخل رقم التواصل" : "Enter contact phone";
        return null;
      default:
        return null;
    }
  ...
  const handleSubmit = async () => {
    ...
    for (const s of sections) {
      const sectionErr = validateSection(s.id);
      if (sectionErr) {
    ...
    try {
      const created = await addListing({
        type: form.type as "apartment" | "villa" | "commercial",
        ...
        beds: form.type === "commercial" ? 0 : Number(form.beds),
  ...
  const inputCls =
    "w-full px-4 py-3.5 rounded-2xl border border-aura-border bg-white text-aura-dark text-sm outline-none focus:border-aura-accent focus:ring-4 focus:ring-aura-accent/10 transition-all duration-300 placeholder:text-aura-muted/50";
              ...
              return (
                ...
                >
                  ...
                  <span className="whitespace-nowrap lg:whitespace-normal text-right">
                    <span className="hidden lg:inline text-aura-muted/60 ml-1">
                      {i + 1}.
                    </span>
      ...
      <div className="lg:col-span-8 xl:col-span-9">
        <div className="bento-card bg-aura-card rounded-3xl border border-aura-border p-6 md:p-8">
          <h3 className="text-xl font-light text-aura-dark mb-6">
            {isAr
              ? sections[sectionIndex].label_ar
              : sections[sectionIndex].label_en}
          </h3>
          ...
          {activeSection === "basic" && (
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-aura-dark">
                    {isAr ? "عنوان الإعلان (عربي)" : "Title (Arabic)"}
                  </label>
                  <input
                    type="text"
                    value={form.title_ar}
                    onChange={(e) => update({ title_ar: e.target.value })}
                    placeholder={isAr ? "مثال: شقة فاخرة بالتجمع الخامس" : "e.g. Luxury apartment"}
                    className={inputCls}
                  />
                </div>
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-aura-dark">
                    {isAr ? "عنوان الإعلان (إنجليزي)" : "Title (English)"}
                  </label>
                  <input
                    type="text"
                    value={form.title_en}
                    onChange={(e) => update({ title_en: e.target.value })}
                    placeholder="Luxury Apartment in New Cairo"
                    className={inputCls}
                    dir="ltr"
                  />
                </div>
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-aura-dark">
                  {isAr ? "وصف الإعلان (عربي)" : "Description (Arabic)"}
                </label>
                <textarea
                  rows={4}
                  value={form.description_ar}
                  onChange={(e) => update({ description_ar: e.target.value })}
                  placeholder={isAr ? "اكتب وصفاً تفصيلياً للعقار..." : "Write a detailed description..."}
                  className={`${inputCls} resize-none`}
                />
              </div>
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-aura-dark">
                  {isAr ? "وصف الإعلان (إنجليزي)" : "Description (English)"}
                </label>
                <textarea
                  rows={4}
          ...
          {activeSection === "location" && (
            <div className="space-y-4">
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-aura-dark">
                  {isAr ? "الموقع (عربي)" : "Location (Arabic)"}
                </label>
                <input
                  type="text"
                  value={form.location_ar}
                  onChange={(e) => update({ location_ar: e.target.value })}
                  placeholder={isAr ? "مثال: التجمع الخامس، القاهرة" : "e.g. New Cairo"}
                  className={inputCls}
                />
              </div>
              <div className="space-y-1.5">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    {form.images.map((src, i) => (
                      <div
                        key={i}
                        className="relative aspect-[4/3] rounded-2xl overflow-hidden border border-aura-border group"
                      >
                        <img
                          src={src}
                          alt={`${i + 1}`}
