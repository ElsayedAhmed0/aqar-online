  ...
  return (
    <>
                    ...
                    <div className="absolute top-full mt-2 left-0 w-56 bg-aura-card border border-aura-border rounded-2xl shadow-xl overflow-hidden z-50">
                      ...
                      {/* الصفحة الشخصية */}
                       ...
                       <a  href={`/${locale}/profile`}
                        ...
                        className="flex items-center gap-3 px-4 py-3 text-xs text-aura-dark hover:bg-aura-canvas transition-colors"
                      >
                      ...
                      <button
                        ...
                        className="w-full flex items-center gap-3 px-4 py-3 text-xs text-red-500 hover:bg-red-50 transition-colors border-t border-aura-border"
                      >
      ...
      >
        ...
        />
        <div
          className={`absolute top-0 left-0 w-80 h-full bg-aura-card p-8 flex flex-col justify-between transition-transform duration-300 ${
        ...
        >
          <div className="space-y-8">
            ...
            <nav className="flex flex-col gap-6">
              ...
              <a href={`/${locale}/contact`} onClick={() => setMobileOpen(false)} className="text-sm font-light text-aura-muted">
                {isAr ? "اتصل بنا" : "Contact"}
              </a>
            </nav>
          </div>
            ...
            {user ? (
              <>
                ...
                <div className="flex items-center gap-3 pb-3 border-b border-aura-border">
                 ...
                 <a  href={`/${locale}/add-listing`}
                  onClick={() => setMobileOpen(false)}
                  className="flex items-center justify-center gap-2 w-full px-6 py-3 rounded-full text-xs font-medium text-white bg-aura-accent transition-all duration-300"
                >
                  <MdOutlineAddHome className="w-4 h-4" />
                  {isAr ? "أضف إعلانك" : "Add Listing"}
                </a>
                 ...
                 <a  href={`/${locale}/profile`}
                  onClick={() => setMobileOpen(false)}
                  className="flex items-center gap-2 w-full px-6 py-3 rounded-full text-xs font-medium text-aura-dark border border-aura-border transition-all duration-300"
                >
                ...
                <button
                  ...
                  className="flex items-center gap-2 w-full px-6 py-3 rounded-full text-xs font-medium text-red-500 border border-red-100 transition-all duration-300"
                >
               ...
               <a  href={`/${locale}/login`}
                onClick={() => setMobileOpen(false)}
                className="block w-full text-center px-6 py-3 rounded-full text-xs font-medium text-white bg-aura-accent transition-all duration-300"
              >
                {isAr ? "تسجيل الدخول" : "Login"}
              </a>
            )}
