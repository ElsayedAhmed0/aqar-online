        ...
        <>
            ...
            {ads.map((ad) => (
                ...
                <a key={ad.id}
                    ...
                    className={`relative rounded-2xl overflow-hidden group cursor-pointer block border border-aura-border hover:border-aura-accent transition-all duration-300 ${inPanel ? "h-36 shadow-sm hover:shadow-md" : "h-48 shadow-md hover:shadow-xl"
                        }`}
                >
                    {/* الصورة */}
                    {ad.image_url ? (
