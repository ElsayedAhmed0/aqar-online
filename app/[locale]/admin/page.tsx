...
type Ad = {
  ...
  image_url: string; link: string; active: boolean; order_num: number;
};
...
const settingsGroups = [
  ...
  {
    title_ar: "قسم الهيرو", title_en: "Hero Section",
    fields: [
  ...
  {
    title_ar: "الإحصائيات", title_en: "Stats",
    fields: [
  ...
  {
    title_ar: "الفوتر", title_en: "Footer",
    fields: [
  ...
  {
    title_ar: "السوشيال ميديا", title_en: "Social Media",
    fields: [
...
const emptyAd = { title_ar: "", title_en: "", subtitle_ar: "", subtitle_en: "", image_url: "", link: "", order_num: "0" };
...
export default function AdminPage() {
  ...
  const saveAd = async () => {
    ...
    const supabase = createClient();
    if (editingAd) {
      const { data } = await supabase.from("ads").update({ ...adForm, order_num: Number(adForm.order_num) }).eq("id", editingAd).select().single();
      if (data) setAds((prev) => prev.map((a) => a.id === editingAd ? data : a));
      setEditingAd(null);
    } else {
      const { data } = await supabase.from("ads").insert({ ...adForm, order_num: Number(adForm.order_num), active: true }).select().single();
          ...
          <div className="flex gap-2 mb-8 bg-aura-card p-1.5 rounded-2xl border border-aura-border w-fit flex-wrap">
            {tabs.map((tab) => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id as any)}
                ...
                {isAr ? tab.ar : tab.en}
                {tab.count !== null && (
                  <span className={`text-xs px-2 py-0.5 rounded-full ${activeTab === tab.id ? "bg-white/20" : "bg-aura-canvas"}`}>{tab.count}</span>
                )}
              </button>
            ))}
          </div>
              ...
              <div className="bento-card bg-aura-card rounded-3xl p-6 border border-aura-border">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {[
                    ...
                    { key: "image_url",   label_ar: "رابط الصورة",       label_en: "Image URL"         },
                  ...
                  {ads.map((ad) => (
                    <div key={ad.id} className="bento-card bg-aura-card rounded-3xl overflow-hidden border border-aura-border">
                      ...
                      <div className="p-5">
                        ...
                        <p className="text-xs text-aura-muted mb-4">{isAr ? ad.subtitle_ar : ad.subtitle_en}</p>
                        <div className="flex gap-2">
                          <button onClick={() => { setEditingAd(ad.id); setAdForm({ title_ar: ad.title_ar, title_en: ad.title_en, subtitle_ar: ad.subtitle_ar, subtitle_en: ad.subtitle_en, image_url: ad.image_url, link: ad.link, order_num: String(ad.order_num) }); }}
