...
export default function AdminPage() {
    ...
    const formatPrice = (price: number) =>
        isAr ? `${(price / 1000000).toFixed(1)} مليون` : `EGP ${(price / 1000000).toFixed(1)}M`;
            ...
            <section className="py-16 lg:py-24 px-6 lg:px-12">
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="flex items-center gap-4 mb-10">
                        ...
                        <div>
                            <p className="text-xs tracking-[0.3em] text-aura-accent uppercase">
                                {isAr ? "لوحة الإدارة" : "Admin Panel"}
                            </p>
                            <h1 className="text-3xl font-light text-aura-dark">
                                {isAr ? "إدارة الموقع" : "Site Management"}
                            </h1>
                        </div>
                    </div>

                    {/* الـ Tabs */}
                    <div className="flex gap-2 mb-8 bg-aura-card p-1.5 rounded-2xl border border-aura-border w-fit">
                        <button
                            onClick={() => setActiveTab("listings")}
                            className={`flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-medium transition-all duration-300 ${activeTab === "listings"
                        ...
                        >
                            ...
                            <span className={`text-xs px-2 py-0.5 rounded-full ${activeTab === "listings" ? "bg-white/20" : "bg-aura-canvas"}`}>
                                {listings.length}
                            </span>
                        </button>
                        <button
                            onClick={() => setActiveTab("users")}
                            className={`flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-medium transition-all duration-300 ${activeTab === "users"
                                    ? "bg-aura-dark text-white"
                                    : "text-aura-muted hover:text-aura-dark"
                                }`}
                        >
                            ...
                            <span className={`text-xs px-2 py-0.5 rounded-full ${activeTab === "users" ? "bg-white/20" : "bg-aura-canvas"}`}>
                                {users.length}
                            </span>
                        </button>
                    </div>
                            ...
                            <div className="grid grid-cols-3 gap-4 mb-8">
                                {(["pending", "approved", "rejected"] as const).map((s) => (
                                    <button
                                        ...
                                        className={`p-4 rounded-2xl border text-center transition-all duration-300 ${listingFilter === s
                        ...
                        <>
                            ...
                            <div className="flex flex-col md:flex-row gap-4 mb-6">
                                <div className="flex gap-2 bg-aura-card p-1.5 rounded-2xl border border-aura-border w-fit">
                                    {(["all", "admin", "user"] as const).map((f) => (
                                        <button
                                            ...
                                            className={`px-4 py-2 rounded-xl text-xs font-medium transition-all duration-300 ${userFilter === f
                                                    ? "bg-aura-dark text-white"
                                                    : "text-aura-muted hover:text-aura-dark"
                                                }`}
                                        >
                                ...
                                <button
                                    ...
                                    className="flex items-center gap-2 px-5 py-3 rounded-2xl bg-aura-dark text-white text-xs font-medium hover:bg-aura-accent transition-all duration-300"
                                >
                                ...
                                <table className="w-full" dir={isAr ? "rtl" : "ltr"}>
                                    <thead>
                                        <tr className="border-b border-aura-border bg-aura-canvas">
                                            <th className="px-6 py-4 text-start text-xs font-medium text-aura-muted">{isAr ? "الاسم" : "Name"}</th>
                                            <th className="px-6 py-4 text-start text-xs font-medium text-aura-muted">{isAr ? "البريد" : "Email"}</th>
                                            <th className="px-6 py-4 text-start text-xs font-medium text-aura-muted">{isAr ? "الهاتف" : "Phone"}</th>
                                            <th className="px-6 py-4 text-start text-xs font-medium text-aura-muted">{isAr ? "الدور" : "Role"}</th>
                                            <th className="px-6 py-4 text-start text-xs font-medium text-aura-muted">{isAr ? "الإعلانات" : "Listings"}</th>
                                            <th className="px-6 py-4 text-start text-xs font-medium text-aura-muted">{isAr ? "تاريخ التسجيل" : "Joined"}</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {filteredUsers.length === 0 ? (
                                            ...
                                            filteredUsers.map((u, i) => (
                                                <tr key={u.id} className={`border-b border-aura-border hover:bg-aura-canvas transition-colors ${i % 2 === 0 ? "" : "bg-aura-canvas/30"}`}>
                                                    ...
                                                    <td className="px-6 py-4">
                                                        <span className={`px-2.5 py-1 rounded-full text-[10px] font-medium ${u.role === "admin"
                                                    ...
                                                    <td className="px-6 py-4 text-sm text-aura-dark text-start">{u.listings_count || 0}</td>
                                                    <td className="px-6 py-4 text-xs text-aura-muted">
                                                        {new Date(u.created_at).toLocaleDateString(isAr ? "ar-EG" : "en-US")}
                                                    </td>
                                                </tr>
                                            ))
                                        )}
